import { findPairsWhichSum } from "../week-eight/kata-06";
...
describe("How do you find all pairs of an integer array whose sum is equal to a given number?", () => {
    ...
    it("Should cope with matches half of goal", () => {
        ...
        expect(pairs.length).toBe(2);
        expect(pairs[0]).toContain(5);
        expect(pairs[0]).toContain(7);

        expect(pairs[1]).toContain(4);
        expect(pairs[1]).toContain(8);
    });
